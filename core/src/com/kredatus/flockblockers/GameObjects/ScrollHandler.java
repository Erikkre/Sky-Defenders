...
public class ScrollHandler {
    ...
    public void update(int boostnumber) {
        ...
        background3.update();
        background4.update();
        //System.out.println("Boost"+i+": "+boostlist.get(i).x+","+boostlist.get(i).y+" scrolled"+boostlist.get(i).isScrolledLeft());
        ...
        if (background3.isScrolledLeft()) {
            background3.reset(background4.getTailX());
        } else if (background4.isScrolledLeft()) {
            background4.reset(background3.getTailX());
        }
    }
    ...
    private void collideboost(ArrayList<Boost> boostlist, Glider glider, int i) {
        if (!boostlist.get(i).isScored()
            ...
            boostlist.get(i).isScrolledLeft = true;   //boost gets restarted if player hits it
    ...
    public ArrayList<Boost> updatelist(ArrayList<Boost> boostlist, boolean flipped, boolean horinv, int boostnumber) {
        ...
        for (int i = 0; i < boostnumber; i++) {
            ...
            if (boostlist.get(i).isScrolledLeft()) {
                ...
                while (same) {   //keep going through loop until the boost that needs to be reset is not the same as another one
                    ...
                    for (Boost j : boostlist) {
                        if (j.getX() == x + background2.getTailX()) {
                            same = true;
                        }
                    }
                    if (!same) {    //if boost isnt already in list add to random x, y using original coords added from rightmost background
                        ...
                        if (!flipped) {
                            boostlist.set(i, new Boost(x + background2.getTailX(), y - (h / boostcoords.get(index).z / 2), (int) width, (int) height));
                        } else {
                            boostlist.set(i, new Boost(x + background2.getTailX(), -y - (h / boostcoords.get(index).z / 2), (int) width, (int) height));
                        }
                    }
    ...
    public void onRestart() {
        ...
        background4.onRestart(background3.getTailX(), -bgh);
